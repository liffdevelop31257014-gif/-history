...
function encodeShareData(profile,cards,torisetsu){
  return base64UrlEncode(JSON.stringify({
    n: profile.name||"", a: profile.age||"", d: createdAt||"",
    c: cards.map(card=>CARD_FIELD_ORDER.map(f=>card[f]||"")),
    t: TORISETSU_QUESTIONS.map(q=>torisetsu[q.id]||""),
    s: schedulePatterns.map(p=>({
      n: p.name||"", l: p.sublabel||"",
      t: (p.slots||[]).map(s=>[s.startTime||"",s.endTime||"",s.activity||""]),
    })),
  }));
}
function decodeShareData(encoded){
  ...
  const cards=(p.c||[]).map(arr=>{
    const obj={}; CARD_FIELD_ORDER.forEach((f,i)=>{ obj[f]=arr[i]||""; }); return obj;
  });
  const torisetsu=Object.fromEntries(TORISETSU_QUESTIONS.map((q,i)=>[q.id,(p.t||[])[i]||""]));
  const schedule=(p.s||[]).map(ps=>({
    id: generateId(), name:ps.n||"", sublabel:ps.l||"",
    slots:(ps.t||[]).map(st=>({id:generateId(),startTime:st[0]||"",endTime:st[1]||"",activity:st[2]||""})),
  }));
  return{profile,cards,torisetsu,schedule,createdAt:p.d||""};
}
...
function getSharedDataFromURL(){
  const raw=new URLSearchParams(location.search).get("share"); if(!raw) return null;
...
function renderContent(container,profile,cards,torisetsu,schedule,opts={}){
  ...
  const schedHasData=(schedule||[]).some(p=>p.slots&&p.slots.length>0);
...
function renderPublicView(shared){
  ...
  renderContent(pv,shared.profile,shared.cards,shared.torisetsu,shared.schedule,{showViewerCTA:true});
}
...
function switchTab(tab){
  ...
  const title={preview:"プレビュー",share:"共有",settings:"設定"};
...
function bindEvents(){
  ...
  document.getElementById("copyUrlBtn").addEventListener("click",async()=>{
    ...
    catch(_){ inp.removeAttribute("readonly"); inp.select(); try{document.execCommand("copy");}catch(_){} inp.setAttribute("readonly","true"); }
  ...
  document.getElementById("lineShareBtn").addEventListener("click",async()=>{
    ...
    await sendShareMessageToSelf(msg);
