...
function bindEvents() {
  ...
  // ※iOSの日付ピッカーは年/月/日を1つずつ確定するたびにchangeが発火し、
  //   途中の値（年がまだ今日の年のまま等）で計算してしまうため、
  //   操作が落ち着くまで少し待ってから最終的な値で計算する。
  let birthdateRecalcTimer = null;
  document.getElementById("profileBirthdate").addEventListener("change", () => {
    clearTimeout(birthdateRecalcTimer);
    birthdateRecalcTimer = setTimeout(() => {
