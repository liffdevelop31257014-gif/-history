...
function bindEvents() {
  ...
  // 生年月日の入力が「完全に終わった」タイミング（フィールドからフォーカスが外れた時）
  // にのみ、年齢だけ入っているカードの年・年だけ入っているカードの年齢を算出する。
  // ※カレンダー/ホイールを開くと最初は「今日の日付」が表示され、年→月→日を
  //   1つずつ選んでいく間にも change が発火する端末があるため、changeでは判定しない。
  //   blur（フィールドを完全に離れた時点）の値だけを正として使う。
